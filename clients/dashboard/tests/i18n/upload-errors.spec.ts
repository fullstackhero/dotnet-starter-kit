...
// wrong interpolation params and every spec stayed green while a Portuguese tenant read English.
//
// Namespacing is the sharp edge here. Every upload surface in this app binds `t` to "files"
// (ImageInput, ProductImageManager, FileDropzone), while the messages live in "common" — so the
// keys the hook raises have to carry the namespace explicitly. Dropping the `common:` prefix
// resolves nothing and renders the last segment of the key instead of a sentence.
//
// The avatar editor on /settings/profile is this app's ImageInput mount, and its picker is an input
// created in JS and clicked programmatically — reachable through the filechooser event, not a
// selector.
...
test.describe("upload failures are localized", () => {
  test("a storage PUT that never connects reports a network error in Portuguese", async ({
    ...
    // Aborting is what xhr.onerror is for; the hook turns it into common:upload.networkError, and
    // the toast resolves it through a `t` bound to "files".
  ...
  test("a storage PUT the bucket rejects reports the status it came back with", async ({ page }) => {
    ...
    // renders fine with the placeholder empty, and the user loses the only diagnostic.
