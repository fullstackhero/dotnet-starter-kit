...
internal static class CatalogSeedData
{
        ...
        IReadOnlyDictionary<string, Category> categoriesByName)
    {
        ...
        return
        [
            Product.Create("ACM-TS-001", "Classic Cotton Tee",      "100% organic cotton crew-neck.",      B("Acme Goods").Id,     C("Tops").Id,         new Money(24.00m,  "USD"),  150, null),
            Product.Create("ACM-HD-002", "Heavyweight Hoodie",      "450gsm fleece pullover hoodie.",      B("Acme Goods").Id,     C("Outerwear").Id,    new Money(68.00m,  "USD"),  60,  null),
            Product.Create("CON-CH-101", "Walnut Lounge Chair",     "Mid-century walnut frame, linen seat.", B("Contoso Studio").Id, C("Furniture").Id,    new Money(489.00m, "USD"),  12,  null),
            Product.Create("CON-LP-102", "Brass Pendant Lamp",      "Hand-finished brass dome pendant.",   B("Contoso Studio").Id, C("Lighting").Id,     new Money(189.00m, "USD"),  24,  null),
            Product.Create("NW-TN-201",  "Trailhead 2P Tent",       "3-season backpacking tent, 2.4kg.",   B("Northwind").Id,      C("Camping").Id,      new Money(279.00m, "USD"),  35,  null),
            Product.Create("NW-SB-202",  "Summit Sleeping Bag",     "Down-filled, comfort to -5°C.",       B("Northwind").Id,      C("Camping").Id,      new Money(219.00m, "USD"),  28,  null),
            Product.Create("FAB-DR-301", "20V Cordless Drill",      "Brushless, 2-speed, 2x batteries.",   B("Fabrikam").Id,       C("Power Tools").Id,  new Money(159.00m, "USD"),  80,  null),
            Product.Create("FAB-WS-302", "16-piece Wrench Set",     "Chrome vanadium, metric + imperial.", B("Fabrikam").Id,       C("Hand Tools").Id,   new Money(72.00m,  "USD"),  120, null),
            Product.Create("FAB-CS-303", "7-1/4\" Circular Saw",    "15-amp corded, 5800 RPM.",            B("Fabrikam").Id,       C("Power Tools").Id,  new Money(129.00m, "USD"),  45,  null),
            Product.Create("ACM-JK-003", "All-Weather Shell",       "3-layer waterproof breathable shell.", B("Acme Goods").Id,    C("Outerwear").Id,    new Money(189.00m, "USD"),  40,  null),
        ];
    }
}
