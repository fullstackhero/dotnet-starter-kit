...
internal sealed class SettingDbInitializer(
    ILogger<SettingDbInitializer> logger,
    SettingDbContext context) : IDbInitializer
{
    public async Task MigrateAsync(CancellationToken cancellationToken)
    {
        if ((await context.Database.GetPendingMigrationsAsync(cancellationToken).ConfigureAwait(false)).Any())
        {
            ...
            logger.LogInformation("[{Tenant}] applied database migrations for Setting module", context.TenantInfo!.Identifier);
        }
    }
