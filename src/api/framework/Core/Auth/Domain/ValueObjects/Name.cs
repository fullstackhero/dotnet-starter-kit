...
public sealed class Name : ValueObject
{
    ...
    public static Result<Name> Create(string value)
    {
        ...
        if (!Regex.IsMatch(value, @"^[a-zA-ZğüşıöçĞÜŞİÖÇ\s-]+$", RegexOptions.None, TimeSpan.FromMilliseconds(200)))
        {
            return Result<Name>.Failure("Name can only contain letters, spaces and dash");
        }

        if (Regex.IsMatch(value, @"\s{2,}", RegexOptions.None, TimeSpan.FromMilliseconds(200)))
        {
