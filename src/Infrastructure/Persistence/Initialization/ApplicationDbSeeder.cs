...
internal class ApplicationDbSeeder
{
    ...
    public ApplicationDbSeeder(FLTenantInfo currentTenant, RoleManager<ApplicationRole> roleManager, UserManager<ApplicationUser> userManager, CustomSeederRunner seederRunner, ILogger<ApplicationDbSeeder> logger)
    {
    ...
    {
        foreach (string roleName in FLRoles.DefaultRoles)
        {
            ...
            // Assign permissions
            if (roleName == FLRoles.Basic)
            {
                await AssignPermissionsToRoleAsync(dbContext, FLPermissions.Basic, role);
            }
            else if (roleName == FLRoles.Admin)
            {
                await AssignPermissionsToRoleAsync(dbContext, FLPermissions.Admin, role);

                if (_currentTenant.Id == MultitenancyConstants.Root.Id)
                {
                    await AssignPermissionsToRoleAsync(dbContext, FLPermissions.Root, role);
                }
            }
        }
    }

    private async Task AssignPermissionsToRoleAsync(ApplicationDbContext dbContext, IReadOnlyList<FLPermission> permissions, ApplicationRole role)
    {
        var currentClaims = await _roleManager.GetClaimsAsync(role);
        foreach (var permission in permissions)
        {
            if (!currentClaims.Any(c => c.Type == FLClaims.Permission && c.Value == permission.Name))
            {
    ...
    private async Task SeedAdminUserAsync()
    {
        ...
        {
            string adminUserName = $"{_currentTenant.Id.Trim()}.{FLRoles.Admin}".ToLowerInvariant();
            adminUser = new ApplicationUser
            {
                FirstName = _currentTenant.Id.Trim().ToLowerInvariant(),
                LastName = FLRoles.Admin,
        ...
        // Assign role to user
        if (!await _userManager.IsInRoleAsync(adminUser, FLRoles.Admin))
        {
            _logger.LogInformation("Assigning Admin Role to Admin User for '{tenantId}' Tenant.", _currentTenant.Id);
            await _userManager.AddToRoleAsync(adminUser, FLRoles.Admin);
        }
    }
}